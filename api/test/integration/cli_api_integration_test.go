...
// Mock handlers
func mockCliAPICreateWorkItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusCreated)
	json.NewEncoder(w).Encode(map[string]string{"id": "WI-456"})
}

func mockCliAPIGetWorkItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(map[string]interface{}{
		"id":        "WI-456",
		"title":     "Test CLI API integration",
		"type":      "TASK",
		"priority":  "MEDIUM",
		"status":    "IN_PROGRESS",
...
func mockCliAPIUpdateWorkItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(map[string]interface{}{
		"id":        "WI-456",
...
// TestCliApiIntegrationV2 verifies integration between the CLI and API
func TestCliApiIntegrationV2(t *testing.T) {
	// Setup router with mock handlers
	r := mux.NewRouter()
	r.HandleFunc("/api/v1/workitems", mockCliAPICreateWorkItem).Methods("POST")
	r.HandleFunc("/api/v1/workitems/{id}", mockCliAPIGetWorkItem).Methods("GET")
	r.HandleFunc("/api/v1/workitems/{id}", mockCliAPIUpdateWorkItem).Methods("PUT")
