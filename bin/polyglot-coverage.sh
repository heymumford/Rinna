...
generate_java_coverage() {
  ...
  if [[ "$VERBOSE" == "true" ]]; then
    ...
    mvn clean test -P jacoco > /dev/null
  fi
...
generate_python_coverage() {
  ...
  if [[ -d "$PROJECT_ROOT/python" ]]; then
    ...
    if [[ -f "$PYTHON_COVERAGE_DIR/coverage.xml" ]]; then
      ...
      python_coverage=$(echo "$coverage_data * 100" | bc -l | xargs printf "%.1f")
...
if [[ $total_files -gt 0 ]]; then
  overall_coverage=$(echo "scale=1; ($java_coverage * $java_files + $go_coverage * $go_files + $python_coverage * $python_files) / $total_files" | bc)
else
  overall_coverage=0
fi
  ...
  cat > "$COVERAGE_DIR/coverage.json" << EOF
{
  ...
  "languages": {
    "java": {
      ...
      "passed": $(( $(echo "$java_coverage >= $JAVA_THRESHOLD" | bc -l) ))
    },
    "go": {
      "coverage": $go_coverage,
      "files": $go_files,
      "threshold": $GO_THRESHOLD,
      "passed": $(( $(echo "$go_coverage >= $GO_THRESHOLD" | bc -l) ))
    },
    "python": {
      "coverage": $python_coverage,
      "files": $python_files,
      "threshold": $PYTHON_THRESHOLD,
      "passed": $(( $(echo "$python_coverage >= $PYTHON_THRESHOLD" | bc -l) ))
    }
  }
}
EOF
...
if (( $(echo "$java_coverage < $JAVA_THRESHOLD" | bc -l) )); then
...
if (( $(echo "$python_coverage < $PYTHON_THRESHOLD" | bc -l) )); then
  echo -e "${RED}⚠️ Python coverage is below threshold!${NC}"
  failed=true
fi

if (( $(echo "$overall_coverage < $FAIL_THRESHOLD" | bc -l) )) && [[ "$FAIL_THRESHOLD" -gt 0 ]]; then
...
if [[ "$failed" == "true" ]] && [[ "$FAIL_THRESHOLD" -gt 0 ]]; then
  echo
