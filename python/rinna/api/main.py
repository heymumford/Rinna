...
class ReportResponse(BaseModel):
    ...
    generation_time_seconds: Optional[float] = Field(None, description="Time taken to generate the report")
...
async def generate_report(request: ReportRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=501, 
            detail=f"Report generation service not available: {e}"
        )
...
async def download_report(report_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=501, 
            detail=f"Report service not available: {e}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
async def get_sample_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get sample metrics: {str(e)}"
        )
