...
class TestCrossLanguageApi(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            if response.status_code != 200:
                raise unittest.SkipTest(f"API server not available: status {response.status_code}")
    ...
    def test_01_create_work_item_from_python(self):
        ...
        # Verify response
        self.assertEqual(201, response.status_code, f"Expected 201 Created, got {response.status_code}")
    ...
    def test_02_retrieve_work_item_with_python(self):
        ...
        # Retrieve the item
        response = requests.get(f"{self.api_url}/workitems/{self.__class__.test_item_id}")
        
        # Verify response
        self.assertEqual(200, response.status_code, f"Expected 200 OK, got {response.status_code}")
        
        # Verify item data
        item = response.json()
        self.assertEqual(self.__class__.test_item_id, item["id"], "ID should match")
        self.assertTrue(item["title"].startswith("Python-Go test item"), "Title should match")
        self.assertEqual("FEATURE", item["type"], "Type should match")
        self.assertEqual("MEDIUM", item["priority"], "Priority should match")
        self.assertEqual("Created by Python test", item["description"], "Description should match")
    ...
    def test_03_update_work_item_from_python(self):
        ...
        get_response = requests.get(f"{self.api_url}/workitems/{self.__class__.test_item_id}")
        updated_item = get_response.json()
        
        self.assertEqual("IN_PROGRESS", updated_item["state"], "State should be updated")
        self.assertTrue("Updated by Python test at" in updated_item["description"], 
    ...
    def test_05_validate_error_handling(self):
        ...
        self.assertEqual(404, response.status_code, "Should return 404 for non-existent item")
        ...
        self.assertGreaterEqual(response.status_code, 400, 
                              "Should return error status for invalid item")
