...
class TestAPIIntegration(unittest.TestCase):
    ...
    def test_api_workflow_integration(self):
        ...
        mock_responses = [
            ...
            MagicMock(status_code=201, json=lambda: {"id": "123", "title": "Test Item"}),
            # Update item response
            MagicMock(status_code=200, json=lambda: {"id": "123", "title": "Test Item", "status": "DONE"})
        ]
...
class WorkflowComponent:
    ...
    def _create_item(self, title):
        ...
        import requests
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.post(f"{self.api_url}/items", headers=headers, json={
            "title": title
        })
        response.raise_for_status()
        return response.json()
    ...
    def _complete_item(self, item_id):
        ...
        import requests
        headers = {"Authorization": f"Bearer {self.token}"}
        response = requests.put(f"{self.api_url}/items/{item_id}", headers=headers, json={
            "status": "DONE"
        })
        response.raise_for_status()
        return response.json()
