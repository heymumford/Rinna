    ...
    def test_search_performance(self):
        ...
        test_cases = [
            {
                "keyword": "Item 1",
                "expected_count": 1111,
            },  # Exact match appears in multiple titles
        ...
        for tc in test_cases:
            ...
            self.assertLess(
                elapsed,
                0.1,
                f"Search for '{tc['keyword']}' took too long: {elapsed:.6f} seconds",
            )
    ...
    def test_filter_performance(self):
        ...
        for tc in test_cases:
            ...
            self.assertLess(
                elapsed,
                0.1,
                f"Filter by {criteria} took too long: {elapsed:.6f} seconds",
            )
    ...
    def test_sort_performance(self):
        ...
        for tc in test_cases:
            ...
            if len(results) > 1:
                if tc["reverse"]:
                    self.assertGreaterEqual(
                        results[0][tc["field"]], results[1][tc["field"]]
                    )
                else:
            ...
            self.assertLess(
                elapsed,
                0.2,
                f"Sort by {tc['field']} took too long: {elapsed:.6f} seconds",
            )
    ...
    def _search_items(self, items, keyword):
        ...
        for item in items:
            if (
                ...
                or (
                    "tags" in item
                    and any(keyword.lower() in tag.lower() for tag in item["tags"])
                )
            ):
                results.append(item)
        return results
