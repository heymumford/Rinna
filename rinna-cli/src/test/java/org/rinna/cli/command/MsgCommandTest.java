...
class MsgCommandTest {
    ...
    void setUp() {
        ...
        Mockito.when(mockServiceManager.getMetadataService()).thenReturn(mockMetadataService);
        ...
        Mockito.when(mockMetadataService.startOperation(anyString(), anyString(), any())).thenReturn("op-123");
        
        // Create command instance for testing
        command = new MsgCommand(mockServiceManager);
    }
    ...
    class HelpDocumentationTests {
        ...
        void shouldDisplayHelpInformationForHelpOperation() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                // Verify metadata tracking for help
        ...
        void shouldDisplayErrorForUnknownOption() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                // Verify metadata tracking for failure
    ...
    class PositiveTestCases {
        ...
        void shouldListMessagesWhenNoSubcommandSpecified() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                verify(mockMetadataService).completeOperation(anyString(), argThat(map -> 
                    ...
                    map.containsKey("count") && ((int)map.get("count")) == 2 &&
                    map.containsKey("action") && map.get("action").equals("list")
                ));
            }
        }
        ...
        void shouldListUnreadMessagesSuccessfully() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                verify(mockMetadataService).completeOperation(anyString(), argThat(map -> 
                    map.containsKey("action") && map.get("action").equals("list_unread")
                ));
            }
        }
        ...
        void shouldListProjectsSuccessfully() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                projects.put("Tracer", Arrays.asList("user1", "user2"));
                projects.put("Quantum", Arrays.asList("user1", "user3"));
                
                Mockito.when(mockMessageService.getAvailableProjects(TEST_USER))
                      .thenReturn(projects);
        ...
        void shouldDeleteMessageSuccessfully() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                // Verify operation tracking
                verify(mockMetadataService).startOperation(eq("message"), eq("DELETE"), any());
                verify(mockMetadataService).completeOperation(anyString(), argThat(map -> 
                    map.containsKey("message_id") && map.get("message_id").equals(TEST_MESSAGE_ID) &&
        ...
        void shouldReplyToMessageSuccessfully() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                verify(mockMetadataService).completeOperation(anyString(), argThat(map -> 
                    map.containsKey("in_reply_to") && map.get("in_reply_to").equals(TEST_MESSAGE_ID) &&
        ...
        void shouldListMessagesFromSpecificSender() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                // Verify operation tracking
                verify(mockMetadataService).startOperation(eq("message"), eq("LIST_FROM_SENDER"), 
                    argThat(map -> map.containsKey("sender") && map.get("sender").equals("sender1")));
            }
        }
        ...
        void shouldListMessagesFromSpecificProjectWithProjectFlag() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                // Verify operation tracking
                verify(mockMetadataService).startOperation(eq("message"), eq("LIST_FROM_PROJECT"), 
                    argThat(map -> map.containsKey("project") && map.get("project").equals("Tracer")));
            }
        }
    ...
    class NegativeTestCases {
        ...
        void shouldShowErrorWhenTryingToReadMessageBelongingToAnotherUser() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                // Verify operation tracking for failure
                verify(mockMetadataService).failOperation(anyString(), any(SecurityException.class));
            }
        }
        ...
        void shouldShowErrorWhenDeleteOperationFails() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                // Verify operation tracking for failure
                verify(mockMetadataService).failOperation(anyString(), any(RuntimeException.class));
            }
        }
    ...
    class ContractTests {
        ...
        @Test
        @DisplayName("Should call MetadataService.startOperation and completeOperation")
        void shouldCallMetadataServiceStartOperationAndCompleteOperation() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                serviceManagerMock.when(ServiceManager::getInstance).thenReturn(mockServiceManager);
                
                // Given
                List<RinnaMessage> messages = new ArrayList<>();
                messages.add(createTestMessage("msg-1", "sender1", TEST_USER, "Test message", TEST_PROJECT, MessageStatus.READ));
    ...
    class IntegrationTests {
        ...
        void shouldWorkWithRealMockMessageServiceImplementation() {
            try (MockedStatic<ServiceManager> serviceManagerMock = Mockito.mockStatic(ServiceManager.class)) {
                ...
                MetadataService realMetadataService = Mockito.mock(MetadataService.class);
                ...
                when(realServiceManager.getMetadataService()).thenReturn(realMetadataService);
                ...
                when(realMetadataService.startOperation(anyString(), anyString(), any())).thenReturn("op-123");
                
                // 1. First send a message
                MsgCommand sendCommand = new MsgCommand(realServiceManager);
                ...
                outContent.reset();
                MsgCommand listCommand = new MsgCommand(realServiceManager);
                ...
                outContent.reset();
                MsgCommand readCommand = new MsgCommand(realServiceManager);
                ...
                // Verify operation tracking was used
                verify(realMetadataService, atLeast(3)).startOperation(anyString(), anyString(), any());
            }
        }
