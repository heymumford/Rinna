...
public class DefaultCognitiveLoadCalculator implements CognitiveLoadCalculator {
    ...
    private void initializeAdjustmentFactors() {
        ...
        typeAdjustmentFactors.put(WorkItemType.STORY, 1.5);
        ...
        paradigmAdjustmentFactors.put(WorkParadigm.ENGINEERING, 1.2);
        paradigmAdjustmentFactors.put(WorkParadigm.PRODUCT, 1.5);
        paradigmAdjustmentFactors.put(WorkParadigm.RESEARCH, 2.0);
    }
    ...
    public int calculateWorkItemLoad(WorkItem workItem) {
        ...
        if (workItem.getCynefinDomain().isPresent()) {
            domainFactor = getDomainLoadFactor(workItem.getCynefinDomain().get());
        }
        
        // Apply paradigm adjustment if available
        double paradigmFactor = 1.0;
        if (workItem.getWorkParadigm().isPresent()) {
            paradigmFactor = getParadigmLoadFactor(workItem.getWorkParadigm().get());
        }
    ...
    public boolean recordActualEffort(UUID workItemId, int estimatedHours, int actualHours) {
        ...
        if (workItem.getCynefinDomain().isPresent()) {
            CynefinDomain domain = workItem.getCynefinDomain().get();
        ...
        if (workItem.getWorkParadigm().isPresent()) {
            WorkParadigm paradigm = workItem.getWorkParadigm().get();
