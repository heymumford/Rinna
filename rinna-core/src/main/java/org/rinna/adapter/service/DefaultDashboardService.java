...
public class DefaultDashboardService implements DashboardService {
    ...
    public List<OverloadRiskData> identifyOverloadRisks() {
        ...
        organizationalUnitRepository.findAll().forEach(unit -> {
            ...
            for (UUID workItemId : unitWorkItems) {
                final int[] tempLoad = {assignedLoad}; // Use array to make it effectively final
                itemRepository.findById(workItemId).ifPresent(item -> {
                    // Calculate the assigned load for each work item
                    tempLoad[0] += cognitiveLoadCalculator.calculateWorkItemLoad(item);
                });
                assignedLoad = tempLoad[0]; // Update the assignedLoad variable
            }
            ...
            for (Map.Entry<String, Integer> memberEntry : memberLoads.entrySet()) {
                ...
                if (memberLoad > memberCapacity * 0.9) { // More than 90% capacity is a risk
                    ...
                    List<OverloadRiskData.WorkItemRisk> contributingItems = new ArrayList<>();
    ...
    public EstimationAccuracyData generateEstimationAccuracyReport(Instant startDate, Instant endDate) {
        ...
        accuracyByParadigm.put(WorkParadigm.EPIC, 80.0);
        accuracyByParadigm.put(WorkParadigm.INITIATIVE, 75.0);
        accuracyByParadigm.put(WorkParadigm.EXPERIMENT, 60.0);
    ...
    private double simulateDomainLoad(CynefinDomain domain, Instant date) {
        ...
        double baseValue = switch (domain) {
            ...
            case COMPLEX -> 20;
            case CHAOTIC -> 10;
            case DISORDER -> 15; // Default value for DISORDER
        };
    ...
    private double simulateParadigmLoad(WorkParadigm paradigm, Instant date) {
        ...
        double baseValue = switch (paradigm) {
            ...
            case INITIATIVE -> 15;
            case EXPERIMENT -> 10;
            case STORY, SPIKE, INCIDENT, MAINTENANCE -> 20; // Default value for other paradigms
        };
    ...
    private Map<String, Double> simulateParadigmLoadBreakdown(WorkParadigm paradigm, Instant date) {
        ...
        switch (paradigm) {
            ...
            case EXPERIMENT:
                ...
                break;
            case STORY:
                breakdown.put("User Stories", 80.0 + 5 * Math.random());
                breakdown.put("Acceptance Criteria", 20.0 + 5 * Math.random());
                break;
            case SPIKE:
                breakdown.put("Investigation", 70.0 + 5 * Math.random());
                breakdown.put("Documentation", 30.0 + 5 * Math.random());
                break;
            case INCIDENT:
                breakdown.put("Resolution", 80.0 + 5 * Math.random());
                breakdown.put("Root Cause Analysis", 20.0 + 5 * Math.random());
                break;
            case MAINTENANCE:
                breakdown.put("Routine Maintenance", 60.0 + 5 * Math.random());
                breakdown.put("Technical Debt", 40.0 + 5 * Math.random());
                break;
        }
    ...
    private Map<WorkParadigm, Integer> simulateParadigmDistribution() {
        ...
        distribution.put(WorkParadigm.EPIC, 18);
        distribution.put(WorkParadigm.INITIATIVE, 3);
        distribution.put(WorkParadigm.EXPERIMENT, 2);
        distribution.put(WorkParadigm.STORY, 8);
        distribution.put(WorkParadigm.SPIKE, 4);
        distribution.put(WorkParadigm.INCIDENT, 2);
        distribution.put(WorkParadigm.MAINTENANCE, 6);
        return distribution;
    }
        ...
        while (currentDate.isBefore(endDate) || currentDate.equals(endDate)) {
            ...
            for (CynefinDomain domain : CynefinDomain.values()) {
                double domainBaseAccuracy = switch (domain) {
                    ...
                    case DISORDER -> baseAccuracy - 10.0; // Default value for DISORDER
                };
            ...
            for (WorkParadigm paradigm : WorkParadigm.values()) {
                double paradigmBaseAccuracy = switch (paradigm) {
                    ...
                    case EPIC -> baseAccuracy + 2.0;
                    case INITIATIVE -> baseAccuracy - 3.0;
                    case EXPERIMENT -> baseAccuracy - 10.0;
                    case STORY -> baseAccuracy + 5.0;
                    case SPIKE -> baseAccuracy - 5.0;
                    case INCIDENT -> baseAccuracy - 8.0;
                    case MAINTENANCE -> baseAccuracy + 8.0;
                };
